%Test getFeature function for autoEncoder DBN in MNIST data set
clc
clear all;
more off;
addpath(genpath('DeepLearnToolboxGPU'));
addpath('DeeBNet');
addpath(genpath('/Volumes/HypoSafe/Thesis/libsvm'));
data = EEG.prepareEEG('+EEG\',60000, 25000); % amountOfData & split
data.normalize('meanvar');
%data.normalize('minmax');

%%
close all;
dbn=DBN();
dbn.dbnType='autoEncoder';
% Turn plot function off in GenerativeRBM.m - line 111

numEpochs=3;


%dbn.dbnType='autoEncoder';
% RBM1
numHid = 200;
rbmParams=RbmParameters(numHid,ValueType.binary);
rbmParams.maxEpoch=numEpochs;
rbmParams.samplingMethodType=SamplingClasses.SamplingMethodType.CD;
rbmParams.performanceMethod='reconstructionScale';
rbmParams.learningRate = 0.04;
rbmParams.hidBias=ones(1,numHid)*(-4);
dbn.addRBM(rbmParams);
% RBM2
% rbmParams=RbmParameters(15,ValueType.binary);
% rbmParams.maxEpoch=numEpochs;
% rbmParams.samplingMethodType=SamplingClasses.SamplingMethodType.CD;
% rbmParams.learningRate = 0.03;
% rbmParams.hidBias=ones(1,20)*(-4);
% rbmParams.performanceMethod='reconstructionScale';
% dbn.addRBM(rbmParams);
% % RBM3
% rbmParams=RbmParameters(100,ValueType.binary);
% rbmParams.maxEpoch=numEpochs;
% rbmParams.samplingMethodType=SamplingClasses.SamplingMethodType.CD;
% rbmParams.performanceMethod='reconstructionScale';

% dbn.addRBM(rbmParams);

%dbn.train(data);
%dbn.backpropagation(data);

%train
ticID=tic;
dbn.train(data);
dbn.backpropagation(data);
toc(ticID)
%%
%test train
trainingFeatures1 = dbn.getFeature(data.trainData);
%disp('Before BP')
%options.MaxIter = 1000000;
% applySVM(trainingFeatures1,data.trainLabels, data.trainTime);
% SVMStruct = svmtrain(trainingFeatures1,data.trainLabels,'Options',...
%         options,'BoxConstraint',0.01);
%SVMStruct = fitcsvm(trainingFeatures1,data.trainLabels,'KernelFunction','rbf');
libsvm_options = '-s 0 -t 2 -c 0.17 -g 0.5';
SVMStruct = svmtrain(data.trainLabels,trainingFeatures1, libsvm_options);
testFeatures1 = dbn.getFeature(data.testData);
%testFeatures2 = dbn.getFeature(data.validationData);
%y_test_est = svmclassify(SVMStruct, testFeatures1);
%y_test_est = predict(SVMStruct, testFeatures1);
y_test_est = svmpredict(data.testLabels, testFeatures1, SVMStruct);
disp('done')
measures = classperf(data.testLabels,y_test_est);

%BP
% ticID=tic;
% dbn.backpropagation(data);
% toc(ticID);
%test after BP
% trainingFeatures2 = dbn.getFeature(data.testData);
% disp('After BP')
% applySVM(trainingFeatures2,data.testLabels);

% ROC Curve
testLabelMinus1 = data.testLabels;
testLabelMinus1(data.testLabels==0) = -1;
auc = plotroc(testLabelMinus1, testFeatures1, SVMStruct);

%% plot
figure;
plotFig={'go' 'r+'};
for i=0:1
    img=data.testData(data.testLabels(:,1)==i,:);
    ext=dbn.getFeature(img);
    plot3(ext(:,1),ext(:,6),ext(:,9),plotFig{i+1});hold on;
end
legend('0','1');
hold off;
%% Reconstruct data

noisyData=data.testData(:,:);%+sqrt(0.02).*randn(size(data.testData(1:9,:)));
[reconstructedData]=dbn.reconstructData(noisyData,5);

noisyData=10.^(data.reverseNormalize(noisyData))-1;
reconstructedData = 10.^(data.reverseNormalize(reconstructedData))-1;

% Backtransform from log10 (-1 since transformation is log(1+spec))
%noisyData = 10.^(noisyData)-1;
%reconstructedData = 10.^(reconstructedData)-1;


%% Plot weights

index = 1:25;
numSplits = 1;
            
plotData = dbn.rbms{1,1}.rbmParams.weight(:,index);
LengthOneCase = length(plotData);
row = LengthOneCase/numSplits;
            
% Create colorbar for all subplots to retain comparability
mini = min(min(plotData));
maxi = max(max(plotData));

for i = 1:25
    img = reshape(plotData(:,i),row,numSplits);
    subplot(5,5,i)
    dispPSD(img,1)
    set(gca,'YDir','normal')
    
    % This sets the limits of the colorbar to manual for the first plot
    %caxis manual
    %caxis([mini maxi]);
    %colorbar;
end

%% Plot PSD

idx = 1;

testsets = [10000 60000 70000 22000 30000 32000];
close all 
 for ii=testsets
       
  subplot(2,6,idx)
  dispPSD(data.testData(ii,:),1);
  subplot(2,6,idx+6)
  dispPSD(reconstructedData(ii,:),1);
  
  idx = idx+1;
 
 end
 
 %% Plot EEG

idx = 1;

close all 
 for ii=4:6
       
  subplot(2,3,idx)
  plotEEG(data.testData(ii,:),3)
  subplot(2,3,idx+3)
  plotEEG(reconstructedData(ii,:),3)
  
  idx = idx+1;
 
 end
%%
 
wake=features_wLabels(features_wLabels(:,1)==0,:);
sleep=features_wLabels(features_wLabels(:,1)==1,:);

meanWake = mean(wake(:,2:end));
meanSleep = mean(sleep(:,2:end));

subplot(1,2,1)
W = dispPSD(meanWake,3);
subplot(1,2,2)
S = dispPSD(meanSleep,3);

% Combined
load('FreqAxes_0to45Hz');
CombinedIMG = [W S];
imagesc([1 3 6],F,10*log10(abs(CombinedIMG))) 
c = colorbar;
xlabel('Time (sec)')
ylabel('Frequency (Hz)')
ylabel(c,'Power/frequency (dB/Hz)')
set(gca,'Ydir','Normal')

%% 
 
subplot(2,3,1)
dispPSD(features_wLabels(5000,2:end),3)
subplot(2,3,2)
dispPSD(features_wLabels(17000,2:end),3)
subplot(2,3,3)
dispPSD(features_wLabels(23500,2:end),3)
subplot(2,3,4)
dispPSD(features_wLabels(10000,2:end),3)
subplot(2,3,5)
dispPSD(features_wLabels(11500,2:end),3)
subplot(2,3,6)
dispPSD(features_wLabels(13000,2:end),3)


 

   